...
const userController = {
  async register(req, res) {
    try {
      ...
      res
        .status(201)
        .json({ message: "Usuario creado con éxito.", usuario: newUser });
    } catch (error) {
    ...
    try {
      ...
      if (!nombre && !correo) {
        return res
          .status(400)
          .json({
            message: "Debes proporcionar un nombre o correo para actualizar.",
          });
      }
  ...
  async resetPassword(req, res) {
    try {
      ...
      const correo = Object.keys(tokens).find(
        (email) => tokens[email] === token
      );
      if (!correo) {
